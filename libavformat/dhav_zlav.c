...
static int zlav_probe(const AVProbeData *p)
{
    if (memcmp(p->buf, "ZLAV", 4))
        return 0;
...
static int read_chunk(AVFormatContext *s)
{
    ...
    while (avio_r8(s->pb) != 'Z' || avio_r8(s->pb) != 'L' || avio_r8(s->pb) != 'A' || avio_r8(s->pb) != 'V') {
        if (avio_feof(s->pb))
            return AVERROR_EOF;
    }
...
static int64_t get_duration(AVFormatContext *s)
{
    ...
    while (offset > 0) {
        if (AV_RL32(buffer + offset) == MKTAG('z','l','a','v')) {
...
static int zlav_read_header(AVFormatContext *s)
{
    ZLAVContext *dhav = s->priv_data;
    uint8_t signature[5];
...
{
    ZLAVStream *dst = s->streams[stream_index]->priv_data;
    ZLAVContext *dhav = s->priv_data;
    struct tm timeinfo;
    time_t t;
...
static int zlav_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ZLAVContext *dhav = s->priv_data;
...
retry:
    ...
    if (dhav->type == 0xfd && dhav->video_stream_index == -1) {
        ...
        case 0x2: st->codecpar->codec_id = AV_CODEC_ID_H264;  break;
        case 0x3: st->codecpar->codec_id = AV_CODEC_ID_HEVC; break;
        ...
        st->priv_data = dst = av_mallocz(sizeof(ZLAVStream));
        if (!st->priv_data)
            return AVERROR(ENOMEM);
        ...
        case 7: st->codecpar->codec_id = AV_CODEC_ID_PCM_S8;    break;
        case 8: st->codecpar->codec_id = AV_CODEC_ID_G729;    break;
        case 10: st->codecpar->codec_id = AV_CODEC_ID_PCM_MULAW; break;
        case 13: st->codecpar->codec_id = AV_CODEC_ID_ADPCM_MS;  break;
        case 14: st->codecpar->codec_id = AV_CODEC_ID_PCM_ALAW;  break;
        case 15: st->codecpar->codec_id = AV_CODEC_ID_PCM_S16LE; break;
        case 16:
        case 17: st->codecpar->codec_id = AV_CODEC_ID_ADPCM_G726;  break;
        case 18: st->codecpar->codec_id = AV_CODEC_ID_AAC;  break;
...
static int zlav_read_seek(AVFormatContext *s, int stream_index,
...
const FFInputFormat ff_zlav_demuxer = {
    .p.name         = "zlav",
    .p.long_name    = NULL_IF_CONFIG_SMALL("Video ZL DAV"),
    .p.extensions   = "dav",
    .p.flags        = AVFMT_GENERIC_INDEX | AVFMT_NO_BYTE_SEEK | AVFMT_TS_DISCONT | AVFMT_TS_NONSTRICT | AVFMT_SEEK_TO_PTS,
    .priv_data_size = sizeof(ZLAVContext),
    .read_probe     = zlav_probe,
    .read_header    = zlav_read_header,
    .read_packet    = zlav_read_packet,
    .read_seek      = zlav_read_seek,
};
